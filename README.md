...
> LoverConnect 的二改分支，在原作者基础上新增 **应用锁** 、 **前台检测**、**夜间巡逻** 功能，优化了**屏幕使用时间**。
> 
...
| 工具 | 功能 |
|------|------|
| night_patrol | 查看夜间前台应用使用报告（默认 1点-3点，可自定义时段） |

**特性：**
- ✅ 基于 queryEvents 精确计算，避免 queryUsageStats 按天汇总的误差
- ✅ 显示各应用夜间使用时长及最后活跃应用
- ✅ 支持自定义起始/结束时间（默认 1:00-3:00）

### 📊 屏幕使用时间（优化）

原生 get_screen_time 已从滚动24小时改为当天 0点到现在。

| 参数 | 说明 |
|------|------|
| endHour | 可选，结束小时（0-23），不传则到当前时间 |
| endMinute | 可选，结束分钟（0-59），默认0 |

数据源已改为 queryEvents 精确计算，不再受 queryUsageStats 按天汇总的影响。
...
- 如果开启了lock的同时录屏，输入密码页面会被录屏的悬浮窗顶掉，当录屏结束后，输入密码页会恢复正常
---
